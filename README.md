# RAG Solution with Click-Through Citations
...
│  │  • Clickable [citation] links rendered inline                       │    │
│  │  • PDF viewer for source documents (rendered in-browser)            │    │
...
│  │                                                                     │    │
│  │  /chat          → RAG chat endpoint (OpenAI Responses API)          │    │
│  │  /content/<path>→ Blob proxy (serves docs via managed identity)     │    │
│  │                                                                     │    │
│  │  Approaches:                                                        │    │
│  │  • ChatReadRetrieveRead: query rewrite → search → answer            │    │
│  │  • Prompt templates (Jinja2) with citation instructions             │    │
│  └──────┬──────────────────┬──────────────────┬────────────────────────┘    │
└─────────┼──────────────────┼──────────────────┼─────────────────────────────┘
          │                  │                  │
          ▼                  ▼                  ▼
...
│                  │ │ Search        │ │                    │
│ • gpt-4.1-mini   │ │ • Vector +    │ │ • Source documents │
│   (chat)         │ │   semantic    │ │ • Served via       │
│ • text-embedding-│ │   search      │ │   backend proxy    │
│   3-small        │ │ • Hybrid      │ │   (managed ID)     │
│   (embeddings)   │ │   ranking     │ │                    │
└──────────────────┘ └───────────────┘ └────────────────────┘
          │
          ▼
...
### How Click-Through Citations Work

1. The LLM references sources using `[filename.pdf]` format in responses
2. The frontend parses these citations and renders them as clickable links
3. Clicking a citation calls the backend `/content/<filename>` endpoint
4. The backend authenticates to Azure Blob Storage using **managed identity** (no keys needed)
5. The file is streamed back to the browser where PDFs are rendered inline with page navigation
6. Users can download the original document directly

This proxy pattern means:
- **No public access** to the storage account is required
- The backend handles authentication transparently
- Network security policies (private endpoints, firewalls) are fully supported
...
- Azure subscription with permissions to create resources and assign RBAC roles

## Deployment Instructions

### Option A: Deploy with New Resources (Fresh Setup)

```bash
# 1. Clone the repo
azd init -t DanielD-GPT/RAG-Solution-with-click-through-citations
...
### Option B: Deploy with Existing Azure Resources

This is the approach used to produce this solution. Use this when you already have Azure OpenAI, AI Search, and Storage provisioned.
...
azd env set AZURE_LOCATION <your-location>  # e.g., eastus2

# 5. Configure Azure OpenAI (set location to where YOUR OpenAI service lives)
azd env set AZURE_OPENAI_SERVICE <your-openai-service-name>
azd env set AZURE_OPENAI_RESOURCE_GROUP <your-resource-group>
azd env set AZURE_OPENAI_LOCATION <openai-service-location>  # e.g., eastus

# 6. Configure Chat Model Deployment
azd env set AZURE_OPENAI_CHATGPT_DEPLOYMENT gpt-4.1-mini
...
# 7. Configure Embedding Deployment
azd env set AZURE_OPENAI_EMB_DEPLOYMENT text-embedding-3-small
...
azd env set AZURE_SEARCH_SERVICE_RESOURCE_GROUP <your-resource-group>
azd env set AZURE_SEARCH_SERVICE_SKU standard

# 9. (Optional) Use an existing search index
azd env set AZURE_SEARCH_INDEX <your-existing-index-name>

# 10. Configure Storage Account
azd env set AZURE_STORAGE_ACCOUNT <your-storage-account-name>
azd env set AZURE_STORAGE_RESOURCE_GROUP <your-resource-group>

# 11. Deploy
azd up
```

> **Important:** The `AZURE_OPENAI_LOCATION` must match the **actual region** of your existing OpenAI service, which may differ from your resource group location. A mismatch will cause a `InvalidResourceLocation` validation error.
...
> **Note:** It may take 5-10 minutes after SUCCESS for the app to be fully available. If you see a "Python Developer" welcome screen, wait and refresh.

## Key Customizations

### Modified System Prompt

The system prompt (`app/backend/approaches/prompts/chat_answer.system.jinja2`) instructs the LLM to provide source-linked references:

> Each source has a name followed by colon and the actual information, always include the source name for each fact you use in the response. Use square brackets to reference the source, for example [info1.txt]. Don't combine sources, list each source separately, for example [info1.txt][info2.pdf]. Each source should be hyperlinked to its file location in Azure storage so that the user can access the actual document.

### Citation Click-Through Architecture

The frontend renders citations as clickable elements. The backend `/content/<path>` endpoint acts as a **secure proxy** to Azure Blob Storage:

- Uses **managed identity** authentication (no storage keys)
- Supports **private endpoint / firewall** configurations
- Renders PDFs inline with page-level navigation
- Supports download of original files

## Configuration Reference

| Environment Variable | Description | Example |
|---|---|---|
| `AZURE_RESOURCE_GROUP` | Resource group name | `rg-my-search-dev` |
| `AZURE_LOCATION` | Primary deployment region | `eastus2` |
| `AZURE_OPENAI_SERVICE` | OpenAI service name | `my-openai-service` |
| `AZURE_OPENAI_LOCATION` | OpenAI service region (must match actual) | `eastus` |
| `AZURE_OPENAI_CHATGPT_DEPLOYMENT` | Chat deployment name | `gpt-4.1-mini` |
| `AZURE_OPENAI_CHATGPT_MODEL` | Chat model name | `gpt-4.1-mini` |
| `AZURE_OPENAI_EMB_DEPLOYMENT` | Embedding deployment name | `text-embedding-3-small` |
| `AZURE_OPENAI_EMB_MODEL_NAME` | Embedding model name | `text-embedding-3-small` |
| `AZURE_OPENAI_EMB_DIMENSIONS` | Embedding dimensions | `1536` |
| `AZURE_SEARCH_SERVICE` | AI Search service name | `my-search-service` |
| `AZURE_SEARCH_SERVICE_SKU` | AI Search SKU | `standard` |
...
If you modify infrastructure (`infra/` folder or `azure.yaml`):
...
When prompted, confirm deletion and permanent purge of resources to avoid ongoing costs.

## Based On

This solution is based on [Azure-Samples/azure-search-openai-demo](https://github.com/Azure-Samples/azure-search-openai-demo) — Microsoft's reference implementation for RAG patterns with Azure OpenAI and Azure AI Search.
<!--
---
